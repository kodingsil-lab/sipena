...
<div class="page-header">
    <h1 class="page-title"><?= esc($pageTitle ?? 'Form Pedoman PPEPP'); ?></h1>
        ...
        <form action="<?= esc($action); ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select name="jenis_dokumen" class="form-select" required>
                        <?php foreach (($jenisOptions ?? []) as $jenis): ?>
                            <option value="<?= esc($jenis); ?>" <?= old('jenis_dokumen', $dokumen['jenis_dokumen'] ?? '') === $jenis ? 'selected' : ''; ?>>
                                <?= esc($jenis); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="col-12">
                    ...
                    <textarea name="deskripsi" class="form-control" rows="4"><?= esc(old('deskripsi', $dokumen['deskripsi'] ?? '')); ?></textarea>
                </div>
                ...
                <div class="col-12">
                    ...
                    <?php if (! empty($dokumen['file_pdf'])): ?>
                        <div class="mt-2">
                            <a href="<?= esc(base_url('uploads/pedoman_ppepp/' . rawurlencode((string) $dokumen['file_pdf']))); ?>" target="_blank" class="btn btn-primary btn-sm">
                                Lihat File Saat Ini
                            </a>
                        </div>
                    <?php endif; ?>
