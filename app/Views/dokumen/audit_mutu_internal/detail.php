...
if (! empty($dokumen['file_pdf'])) {
    $pdfUrl = base_url('/uploads/audit_mutu_internal/' . rawurlencode($dokumen['file_pdf']));
}
?>
...
<div class="detail-wrap">
    <div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            <h1 class="page-title mb-1"><?= esc($pageTitle ?? 'Detail Audit Mutu Internal'); ?></h1>
            <p class="page-subtitle mb-0"><?= esc($pageDesc ?? ''); ?></p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            <a href="<?= base_url('/audit-mutu-internal/edit/' . $dokumen['id']); ?>" class="btn btn-warning">Edit</a>
                ...
                <div class="card-body p-4">
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <div><?= esc($dokumen['judul'] ?? '-'); ?></div>
                        </div>

                        <div class="col-12">
                            <label class="form-label fw-bold">Nomor Dokumen</label>
                            <div><?= esc($dokumen['nomor_dokumen'] ?? '-'); ?></div>
                        </div>

                        <div class="col-12">
                            <label class="form-label fw-bold">Tahun</label>
                            <div><?= esc($dokumen['tahun'] ?? '-'); ?></div>
                        </div>
