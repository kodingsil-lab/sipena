...
<div class="card card-clean mb-4">
    <div class="card-body p-4">
        <form action="<?= base_url('/peraturan'); ?>" method="get" class="row g-3 align-items-end">
            ...
            <div class="col-lg-2 col-md-2">
                ...
                <select name="kategori" class="form-select">
                    ...
                    <?php foreach (($daftarKategori ?? []) as $kategori): ?>
                        <option value="<?= esc($kategori); ?>" <?= ($kategoriAktif ?? '') === $kategori ? 'selected' : ''; ?>>
                            <?= esc($kategori); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="col-lg-2 col-md-12">
                <div class="d-flex gap-2">
                    ...
                    <a href="<?= base_url('/peraturan'); ?>" class="btn btn-secondary">Reset</a>
                </div>
            </div>
        </form>
    </div>
            ...
            <table class="table table-hover mb-0 align-middle">
                ...
                <tbody>
                    <?php if (! empty($peraturan)): ?>
                        <?php $startNo = (((int) (($pager ?? null) ? $pager->getCurrentPage('peraturan') : 1)) - 1) * ((int) ($perPage ?? 15)); ?>
                        <?php foreach ($peraturan as $i => $item): ?>
                            <tr>
                                <td><?= $startNo + $i + 1; ?></td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" class="text-center text-muted py-4">Belum ada data peraturan.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    <div class="card-footer bg-white border-0 pt-0 pb-3 px-3">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <small class="text-muted">
                Menampilkan <?= count($peraturan); ?> data pada halaman ini
                <?php if (! empty($pager)): ?>
                    (Halaman <?= (int) $pager->getCurrentPage('peraturan'); ?> dari <?= (int) $pager->getPageCount('peraturan'); ?>)
                <?php endif; ?>
            </small>
            <?php if (! empty($pager)): ?>
                <?= $pager->only(['keyword', 'kategori', 'status', 'per_page'])->links('peraturan', 'default_full'); ?>
            <?php endif; ?>
        </div>
    </div>
</div>
