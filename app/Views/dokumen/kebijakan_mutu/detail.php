...
if (! empty($kebijakan['file_pdf'])) {
    $pdfUrl = base_url('/uploads/kebijakan_mutu/' . rawurlencode($kebijakan['file_pdf']));
}
?>
...
<div class="detail-wrap">
    <div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            <h1 class="page-title mb-1"><?= esc($pageTitle ?? 'Detail Kebijakan Mutu'); ?></h1>
            <p class="page-subtitle mb-0"><?= esc($pageDesc ?? ''); ?></p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            <?php if ($canEditDokumen): ?>
                <a href="<?= base_url('/kebijakan-mutu/edit/' . $kebijakan['id']); ?>" class="btn btn-warning">Edit</a>
            <?php endif; ?>
