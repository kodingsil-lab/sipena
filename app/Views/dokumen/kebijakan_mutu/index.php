...
<div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-3">
    <div>
        <h1 class="page-title"><?= esc($pageTitle ?? 'Kebijakan Mutu'); ?></h1>
        <p class="page-subtitle"><?= esc($pageDesc ?? ''); ?></p>
    </div>
    <div>
        <a href="<?= base_url('/kebijakan-mutu/tambah'); ?>" class="btn btn-primary">Tambah Dokumen</a>
    </div>
</div>
...
<div class="card card-clean mb-4">
    <div class="card-body p-4">
        <form action="<?= base_url('/kebijakan-mutu'); ?>" method="get" class="row g-3 align-items-end">
            ...
            <div class="col-lg-2 col-md-12">
                <div class="d-flex gap-2">
                    ...
                    <a href="<?= base_url('/kebijakan-mutu'); ?>" class="btn btn-secondary">Reset</a>
                </div>
            </div>
        </form>
    </div>
            ...
            <table class="table table-hover mb-0 align-middle">
                ...
                <tbody>
                    <?php if (! empty($kebijakan)): ?>
                        <?php $startNo = (((int) (($pager ?? null) ? $pager->getCurrentPage('kebijakan_mutu') : 1)) - 1) * ((int) ($perPage ?? 15)); ?>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="action-buttons">
                                        <a href="<?= base_url('/kebijakan-mutu/detail/' . $item['id']); ?>" class="action-icon-btn action-view" data-bs-toggle="tooltip" title="Detail" aria-label="Detail">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="<?= base_url('/kebijakan-mutu/edit/' . $item['id']); ?>" class="action-icon-btn action-edit" data-bs-toggle="tooltip" title="Edit" aria-label="Edit">
                                            <i class="bi bi-pencil-square"></i>
                                        </a>

                                        <form action="<?= base_url('/kebijakan-mutu/hapus/' . $item['id']); ?>" method="post" class="d-inline">
                                            <?= csrf_field(); ?>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" class="text-center text-muted py-4">Belum ada data Kebijakan Mutu.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    <div class="card-footer bg-white border-0 pt-0 pb-3 px-3">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <small class="text-muted">
                ...
                <?php if (! empty($pager)): ?>
                    (Halaman <?= (int) $pager->getCurrentPage('kebijakan_mutu'); ?> dari <?= (int) $pager->getPageCount('kebijakan_mutu'); ?>)
                <?php endif; ?>
            </small>
            <?php if (! empty($pager)): ?>
                <?= $pager->only(['keyword', 'status', 'per_page'])->links('kebijakan_mutu', 'default_full'); ?>
            <?php endif; ?>
        </div>
    </div>
</div>
